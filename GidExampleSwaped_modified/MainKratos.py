...
class test_utility_get_elements_ids(FluidDynamicsAnalysisROM):

    def FinalizeSolutionStep(self):
        super().FinalizeSolutionStep()
        MainModelPart = self._solver.GetComputingModelPart().GetParentModelPart()
        #declearing submodelparts
        inlet_model_part = MainModelPart.GetSubModelPart('AutomaticInlet2D_Inlet')
        outlet_model_part = MainModelPart.GetSubModelPart('Outlet2D_Outlet')
        no_slip_cylinder = MainModelPart.GetSubModelPart('NoSlip2D_No_Slip_Cylinder')
        np_slip_walls = MainModelPart.GetSubModelPart('NoSlip2D_No_Slip_Walls')
        #finding submodelparts' elements
        requested_elements_inlet = np.array(self.ResidualUtilityObject.GetElementsIds(MainModelPart, inlet_model_part))
        requested_elements_outlet = np.array(self.ResidualUtilityObject.GetElementsIds(MainModelPart, outlet_model_part))
        requested_elements_cylinder = np.array(self.ResidualUtilityObject.GetElementsIds(MainModelPart, no_slip_cylinder))
        requested_elements_walls = np.array(self.ResidualUtilityObject.GetElementsIds(MainModelPart, np_slip_walls))
        #merging into single np_array
        global_requested_elements = np.concatenate((requested_elements_inlet, requested_elements_outlet), axis=None)
        global_requested_elements = np.concatenate((global_requested_elements, requested_elements_cylinder), axis=None)
        global_requested_elements = np.concatenate((global_requested_elements, requested_elements_walls), axis=None)
        #make unique
        global_requested_elements = np.unique(global_requested_elements)

        np.save('element_ids.npy', global_requested_elements)
...
def GenerateIds():
    with open("ProjectParameters.json",'r') as parameter_file:
        parameters = KratosMultiphysics.Parameters(parameter_file.read())
    model = KratosMultiphysics.Model()
    simulation = test_utility_get_elements_ids(model,parameters,"EmpiricalCubature")
    simulation.Run()
